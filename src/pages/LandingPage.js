...
const Dashboard = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error('Error while getting sop');
      console.error('Error during submission:', error);
    } finally {
      setIsLoading(false);
      stopTimer(intervalId, setTimerRunning);
  ...
  const handleGenerateCustomerSOPClick = async () => {
    if (customerSpecificValue.trim() === '') {
      if(customerSpecificLeves.length<=0){
        toast.error('Level is not found for this project');
        return
      }
      toast.error('Please select level value');
      return;
    } else {
      isAiGenrated(false);
      setTimer(0);
      ...
      {aiGenrated && (
        <>
          <div className="customer_specific_section">
            <div>
              <button onClick={() => getProjectSpecificLevel()}>Customer Specific SOP</button>
            </div>
