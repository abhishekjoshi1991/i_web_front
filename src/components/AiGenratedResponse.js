...
import { setEditedOriginalResponse } from '../toolkit/genratedSopSlice';
const AiGenratedResponse = () => {
  ...
  const handleImprovementChange = useCallback(
    ...
    [dispatch, isCustomerSpecific] // Make sure the dependencies are accurate
  );
  return (
    <div className='ai_genrated_response'>
      ...
      <div className="html-sop-buttons">
        <button onClick={handleCorrectSop}>
          <FaCheckCircle size={'1.7rem'} /> Correct
        </button>
        <button onClick={handleShowModal}>
          <FaPencilRuler size={'1.6rem'} /> Need Improvement
        </button>
      </div>
      ...
      <Modal show={showModal} onHide={handleCloseModal} dialogClassName="custom-modal-width">
        <Modal.Header closeButton>
          <div>
            <Modal.Title>Generated SOP</Modal.Title>
          </div>
        </Modal.Header>

        <Modal.Body>
          <p>(Modify this SOP if required)</p>
          <textarea
          ...
          <textarea
            className="form-control"
            placeholder="Enter your feedback and improvement here"
            value={feedback}
